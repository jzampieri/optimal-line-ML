...
def simulate_generation(cars, track_mask, background_path="data/image.png", wait_at_end=True):
    start_point = np.array([70, 680], dtype=np.float32)
    end_point = np.array([700, 120], dtype=np.float32)
    positions = generate_grid_positions(start_point, len(cars), track_mask, spacing=12)
    ...
    for step_i in range(max_steps):
        for idx, car in enumerate(cars):
            ...
            distance_to_goal = np.linalg.norm(next_pos - end_point)
    ...
    for idx, car in enumerate(cars):
        distance_to_goal = np.linalg.norm(positions[idx] - end_point)
        car.fitness = -distance_to_goal + car.distance - (car.penalties * 10)
        print(f"🔵 Car {idx}: Distance = {car.distance:.2f} m | To Goal: {distance_to_goal:.2f} | Penalties: {car.penalties} | Fitness: {car.fitness:.2f}")
